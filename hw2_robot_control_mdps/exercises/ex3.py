...
def reset_robot(default_qpos: np.ndarray) -> np.ndarray:
    ...
    """
    # Add small uniform noise in range (-0.5, 0.5) to default_qpos
...
def reset_target_position(base_pos: np.ndarray) -> np.ndarray:
    ...
    Returns:
    - target_pos: np.ndarray. The absolute 3D position of the target (base + offset). Dimensionality: 1D array, Shape: (3,).
    """
    # Sample new target position offset from uniform distribution
    x_offset = np.random.uniform(0.2, 0.4)
    y_offset = np.random.uniform(-0.2, 0.2)
    z_offset = np.random.uniform(0.1, 0.4)
    relative_pos = np.array([x_offset, y_offset, z_offset])
    # Return absolute position in world frame
...
def process_action(action: np.ndarray, jnt_range: np.ndarray) -> np.ndarray:
    ...
    """
    # Map normalized action [-1, 1] to joint range [low, high]
    # Formula: mid + action * half_range, where mid = (low + high) / 2
    mid = (jnt_range[:, 0] + jnt_range[:, 1]) / 2.0
    half_range = (jnt_range[:, 1] - jnt_range[:, 0]) / 2.0
...
def get_obs(qpos: np.ndarray, ee_pos_w: np.ndarray, ee_rot_w: np.ndarray, base_pos_w: np.ndarray, base_rot_w: np.ndarray, target_pos_w: np.ndarray) -> np.ndarray:
    ...
    # Transform positions to base frame (subtract base position)
    ee_pos_base = ee_pos_w - base_pos_w
    target_pos_base = target_pos_w - base_pos_w

    # Transform EE orientation to base frame: q_base^ee = conj(q_base^world) * q_ee^world
